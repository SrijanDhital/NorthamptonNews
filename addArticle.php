		...
		$pagecontent = $pagecontent . '
  		...
  		if(isset($_POST['submit'])){
	  		if (isset($_POST['title'], $_POST['category'], $_POST['content'])) {
	  			...
	  			$image_path = 'articleimages/' . $image . '';
	  			$tmp_dir = $_FILES['image']['tmp_name'];

	  			$publisher_id = $_SESSION['admin_id'];

	  			//set the name of admin as a publisher for that article
	  			$convert_id = $pdo->prepare('SELECT email FROM admins WHERE admin_id="' . $publisher_id . '"');
			      $convert_id -> execute();
	  			...
	  			$stmt = $pdo->prepare('INSERT INTO articles (title, category, content, published_date, image, publisher) VALUES ( :title, :category, :content, :published_date, :image, :publisher)');

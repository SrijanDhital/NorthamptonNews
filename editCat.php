...
//record of the selected category can be edited from this page
	...
	<form class="form" action="editCat.php?editName=' . $currentName . '" method="post" enctype="multipart/form-data">
	...
	if(isset($_POST['submit'])){
		if (isset($_POST['newName'], $_POST['newDesc'])) {
			...
			if($newName != ""){
			    ...
			    $stmt = $pdo->prepare('UPDATE categories SET name = "' . $newName . '" WHERE name = "' . $currentName . '"');
			...
			if($newDesc != ""){
				...
				$stmt2=$pdo->prepare('UPDATE categories SET description = "' . $newDesc . '" WHERE name = "' . $currentName . '"');
				$stmt2->execute();

		...
		if(isset($_POST['submit'])){
			if (isset($_POST['newfname'], $_POST['newlname'], $_POST['newpass'])) {
				...
				if($newfname != ""){ //change firstname when user inputs a new firstname
				    ...
				    $stmt = $pdo->prepare('UPDATE admins SET firstname = "' . $newfname . '" WHERE email = "' . $currentEmail . '"');
				...
				if($newlname != ""){ //change lastname when user inputs a new lastname
					...
					$stmt2=$pdo->prepare('UPDATE admins SET lastname = "' . $newlname . '" WHERE email = "' . $currentEmail . '"');
				...
				if($pass != ""){ //change password when user inputs a new one
					...
					$stmt3=$pdo->prepare('UPDATE admins SET password = "' . $newpass . '" WHERE email = "' . $currentEmail . '"');

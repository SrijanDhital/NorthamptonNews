...
//record of the selected article can be edited from this page
	...
	if(isset($_POST['submit'])){
		if(isset($_POST['newtitle'], $_POST['newcategory'], $_POST['newcontent'])) {
			...
			$stmt2=$pdo->query('UPDATE articles SET category = "' . $newcategory . '" WHERE title = "' . $currenttitle . '"');
			$stmt2->execute();
			...
			if($newtitle != ""){
			    ...
			    $stmt3=$pdo->prepare('UPDATE articles SET title = "' . $newtitle . '" WHERE title = "' . $currenttitle . '"');
			...
			if($newcontent != ""){
				...
				$stmt4=$pdo->prepare('UPDATE articles SET content = "' . $newcontent . '" WHERE title = "' . $currenttitle . '"');
			    $stmt4->execute();
